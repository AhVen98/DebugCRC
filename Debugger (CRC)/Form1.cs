...
namespace Debugger__CRC_
{
    public partial class frmCRC : Form
    {
        ...
        // Méthode qui calcule le CRC d'une donnée.
        {
            ...
            int CRCVal = 0; //initialisation 

            // Parcourir la donnée
            for (int i = 0; i < Donnée.Length; i++)
            {
                char c = Donnée[i];
                if ((c >= '0') && (c <= '9')) // C'est un chiffre compris entre 0 et 9, compris
                {
                    NbChiffres++;
                    CRCVal = CRCVal + ((int)c - (int)'0');
                    if (CRCVal >= 100) // On ne peut pas dépasser 100 parce qu'on n'a que deux chiffres pour le CRC, mais ça veut dire que le 100 doit aussi être limité
                        CRCVal = CRCVal - 100;
                }
            }
